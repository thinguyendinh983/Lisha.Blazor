...
namespace Lisha.Blazor.Shared
{
    public partial class MainLayout
    {
        ...
        private void Logout()
        {
            ...
            {
                { nameof(Components.Dialogs.Logout.ContentText), $"{L["Logout Confirmation"]}"},
                { nameof(Components.Dialogs.Logout.ButtonText), $"{L["Logout"]}"},
                { nameof(Components.Dialogs.Logout.Color), Color.Error}
            };

            var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Small, FullWidth = true };
            DialogService.Show<Components.Dialogs.Logout>(L["Logout"], parameters, options);
        }
